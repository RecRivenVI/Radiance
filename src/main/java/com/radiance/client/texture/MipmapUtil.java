        ...
        for (int i = 1; i <= targetLevel; i++) {
            ...
            for (int x = 0; x < newWidth; x++) {
                for (int y = 0; y < newHeight; y++) {
                    nextLevel.setPixelRGBA(x, y,
                        blend(currentSource.getPixelRGBA(x * 2, y * 2),
                            currentSource.getPixelRGBA(x * 2 + 1, y * 2),
                            currentSource.getPixelRGBA(x * 2, y * 2 + 1),
                            currentSource.getPixelRGBA(x * 2 + 1, y * 2 + 1), bl));
                }
            }
        ...
        for (int level = 1; level <= maxLevel; level++) {
            ...
            for (int x = 0; x < next.getWidth(); x++) {
                for (int y = 0; y < next.getHeight(); y++) {
                    next.setPixelRGBA(x, y, blend(previous.getPixelRGBA(x * 2, y * 2),
                        previous.getPixelRGBA(x * 2 + 1, y * 2),
                        previous.getPixelRGBA(x * 2, y * 2 + 1),
                        previous.getPixelRGBA(x * 2 + 1, y * 2 + 1), hasAlpha));
                }
            }
        ...
        if (mipmap + 1 <= originals.length) {
            ...
            for (int i = 1; i <= mipmap; i++) {
                if (i < originals.length) {
                    ...
                    for (int l = 0; l < j; l++) {
                        for (int m = 0; m < k; m++) {
                            nativeImage2.setPixelRGBA(l, m,
                                blend(nativeImage.getPixelRGBA(l * 2, m * 2),
                                    nativeImage.getPixelRGBA(l * 2 + 1, m * 2),
                                    nativeImage.getPixelRGBA(l * 2, m * 2 + 1),
                                    nativeImage.getPixelRGBA(l * 2 + 1, m * 2 + 1), bl));
                        }
                    }
    ...
    public static boolean hasAlpha(NativeImage image) {
        for (int i = 0; i < image.getWidth(); i++) {
            for (int j = 0; j < image.getHeight(); j++) {
                if (ARGB.alpha(image.getPixelRGBA(i, j)) == 0) {
                    return true;
                }
            }
        }
